...
public class DataManager {
    ...
    public void updateNewsWire(String section) {
        Intent requestIntent = new Intent(mContext, DownloadService.class);
        requestIntent.putExtra(
                DownloadService.KEY_URL,
                "http://api.nytimes.com/svc/news/v3/content/all/all/.json?api-key=ce28bf606898b473be1a4cdd17ee165a%3A16%3A72738095");
        requestIntent.putExtra(DownloadService.KEY_BROADCAST_ACTION, BROADCAST_ACTION_NEWS_UPDATE);
    ...
    private class NewsWireReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getBooleanExtra(DownloadService.KEY_IS_RESULT_OK, false)) {
                JSONObject newsJSONObj = null;

                try {
                    newsJSONObj = new JSONObject(
                            intent.getStringExtra(DownloadService.KEY_RESULT_JSON));
            ...
            notifyNewsWireUpdateFailed(intent.getStringExtra(DownloadService.KEY_ERROR_MESSAGE));
        }
    }
    //----------------------------------------------------------------------------------------------
}
